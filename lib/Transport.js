...
function _getAsyncRequestWrapper(req) {
    return function (done) {
        ...
        try {
            ...
            log.debug("[Transport] Requesting URL: " + req.url + ", headers: " + req.headers + ", method: " + req.method + ", body(when POST method): " + req.body + ", timeout: " + req.timeout);
            ...
            log.error("[Transport] exception parsing the body or in request, ex: " + ex);
            done(ex);
        }
    };
}
...
function _processResponse(error, response, body, originalRequest) {
    ...
    if (error || !response) {
        log.error("[Transport] _processResponse no response or error with originalRequest.url: " + originalRequest.url + ", IP: " + originalRequest.ip(error ? " Error Code: " + error.code : ""));
        result = CONST.RESPONSE_TYPE.ERROR_API_URL;
    } else {
        try {
            log.debug("[Transport] _processResponse body: " + body + " originalRequest.url: " + originalRequest.url);
