...
function _validateRequests(requests) {
    ...
    requests.forEach(function (req) {
        ...
        if (validator.isValidRequest(clonedRequest)) {
            if (!requestNames[clonedRequest.name]) {
                if ((clonedRequest.body && typeof clonedRequest.body === "object") || (clonedRequest.data && typeof clonedRequest.data === "object")) {
                    clonedRequest.json = true;
                }
                batchRequests.valid.push(validator.cleanRequest(clonedRequest));
                requestNames[clonedRequest.name] = true;
            }
            else {
                batchRequests.invalid[clonedRequest.name] = CONST.RESPONSE_TYPE.DUPLICATE_NAMES;
            }
        }
        else {
            batchRequests.invalid[clonedRequest.name || "missingName"] = CONST.RESPONSE_TYPE.INVALID_TASK;
        }
    });
