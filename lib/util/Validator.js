...
function isValidRequest(req) {
    log.debug(logEntity + " validator isValidRequest req=" + JSON.stringify(req));
    req = req || {};
    var valid = false;
    ...
    log.debug(logEntity + " validator isValidRequest: " + valid);
    return valid;
}
...
function _isValidURL(url) {
    ...
    if (typeof url === "string") {
        log.debug(logEntity + " validator URL to validate: " + url);
...
function _isInWhiteList(host) {
    ...
    if(Array.isArray(whiteList)) {
        if (whiteList.indexOf("*") > -1) {//Supports all URLs
            log.debug(logEntity + " validator Allow all URLS");
            valid = true;
        } else {
            try {
                ...
                log.error(logEntity + " validator isInWhiteList, exception e: " + e + " host: " + host);
            }
        }
    }else{
