...
function _process(req, error, response, body, callback) {
    ...
    if (!response) {
        ...
        if (config.originalHeader) {
            // if using originalHeader don't pass the header - don't want to pass double data
            delete _result.headers;
            _result.originalHeader = _result.headers || response.headers;
        }
